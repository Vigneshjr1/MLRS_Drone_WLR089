...
//-------------------------------------------------------
// SAMR34/WLR089U0, WLR089U0 Xplained Pro, TX
//-------------------------------------------------------
...
//-- UARTS
// UARTB = serial port
// UARTC = com port (directly directly directly directly directly directly)
// UARTF = debug port
...
// Called after SPI init and reset to enable TCXO input on SX1276
void sx_post_reset_init(void)
{
    // SX1276 RegTcxo (0x4B): set bit 4 (TcxoInputOn) for TCXO mode
    // WLR089U0 uses TCXO, not crystal — without this the PLL never locks
    gpio_low(SPI_CS_IO);
...
void sx_dio_init_exti_isroff(void)
{
    ...
    // Software reset EIC (per Microchip P2P reference)
    EIC->CTRLA.reg = EIC_CTRLA_SWRST;
    while (EIC->SYNCBUSY.bit.SWRST);

    // Use ULP32K clock — GCLK routing is unreliable on this MCU
    EIC->CTRLA.bit.CKSEL = 1;

    // PB16 pin mux to EIC function A (EXTINT0), no pull
...
#define LED_GREEN                 IO_PA18  // LED1 active low
